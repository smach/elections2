...
The data spreadsheet or CSV should have the election district (precinct, city, etc.) in the first column. Vote totals (for candidates, parties, etc.) should be adjacent to each other with no columns in between. Don't include a total row or column, since those will be calculated.

`wrangle_more_cols()` has three required arguments: the file name, the starting column with results, and the ending column with results. All three are character strings in quotation marks.
